...
export function ReviewsSection({ productId }: { productId: number }) {
  ...
  // Chargement initial — dépend uniquement de productId, pas d'une fonction externe
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true);
      setShowForm(false);
      // Recharge la liste après un nouvel avis (n'apparaîtra qu'une fois approuvé,
      // mais le summary/reviews restent cohérents si l'utilisateur revient plus tard)
      reviewService.getByProduct(productId).then(setReviews);
      reviewService.getSummary(productId).then(setSummary);
    } finally {
      setSubmitting(false);
